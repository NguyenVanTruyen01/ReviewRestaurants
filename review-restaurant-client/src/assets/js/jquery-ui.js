(function (factory) {
	...
	$.Widget.prototype = {
		...
		_setOptionClasses: function (value) {
			...
			for (classKey in value) {
				...
				elements.addClass(
					this._classes({ element: elements, keys: classKey, classes: value, add: !0 })
				);
			}
		},
			...
			$.each(handlers, function (event, handler) {
				function handlerProxy() {
					if (
						...
						(instance.options.disabled === !0 || $(this).hasClass("ui-state-disabled"))
					) {
						return;
					}
		...
		_trigger: function (type, event, data) {
			...
			return !(
				($.isFunction(callback) && callback.apply(this.element[0], [event].concat(data)) === !1) ||
	...
	var effectsEffectFade = $.effects.define("fade", "toggle", function (options, done) {
		...
		$(this)
			...
			.animate(
				{ opacity: show ? 1 : 0 },
				{ queue: !1, duration: options.duration, easing: options.easing, complete: done }
			);
	});
		...
		_refreshValue: function () {
			...
			if (this._hasMultipleValues()) {
				...
				if (oRange === "max" && this.orientation === "horizontal") {
					this.range
						.stop(1, 1)
						[animate ? "animate" : "css"]({ width: 100 - valPercent + "%" }, o.animate);
				}
				...
				if (oRange === "max" && this.orientation === "vertical") {
					this.range
						.stop(1, 1)
						[animate ? "animate" : "css"]({ height: 100 - valPercent + "%" }, o.animate);
				}
			}
		},
		_handleEvents: {
	...
	var widgetsSortable = $.widget("ui.sortable", $.ui.mouse, {
		...
		_mouseDrag: function (event) {
			...
			for (i = this.items.length - 1; i >= 0; i--) {
				...
				if (
					...
					(this.options.type === "semi-dynamic" ? !$.contains(this.element[0], itemElement) : !0)
				) {
		...
		cancel: function () {
			...
			if (this.placeholder) {
				...
				$.extend(this, { helper: null, dragging: !1, reverting: !1, _noFinalSort: null });
	...
	$.widget("ui.spinner", {
		...
		_spin: function (step, event) {
			...
			if (!this.spinning || this._trigger("spin", event, { value: value }) !== !1) {
				this._value(value);
				this.counter++;
			}
		},
